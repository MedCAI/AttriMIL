...
def train_dsmil(datasets,
                save_path='./save_weights/camelyon16_dsmil_imagenet/',
                feature_dim = 512,
                n_classes = 2,
                fold = 0,
                writer_flag = True,
    ...
    model = MILNet(feature_dim=feature_dim, 
                   n_classes=n_classes)
    _ = model.to(device)
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer, loss_fn):
    ...
    for batch_idx, (data, label, coords, nearest) in enumerate(loader):
        ...
        acc_logger.log(Y_hat, label)
        max_prediction, _ = torch.max(ins_prediction, 0)
        loss_bag = loss_fn(bag_prediction.view(1, -1), label)
        loss_ins = loss_fn(max_prediction.view(1, -1), label)
        loss = 0.5 * loss_bag + 0.5 * loss_ins
        ...
        if (batch_idx + 1) % 20 == 0:
            print('batch {}, loss: {:.4f}, loss_bag: {:.4f}, loss_ins: {:.4f}, bag_size: {}'.format(batch_idx, loss_value, loss_bag_value, loss_ins_value, label.item(), data.size(0)))
    ...
    print('Epoch: {}, train_loss: {:.4f}, bag_loss: {:.4f}, ins_loss: {:.4f}'.format(epoch, train_loss, bag_loss, ins_loss))
    ...
    if writer:
        ...
        writer.add_scalar('train/loss_ins', loss_ins, epoch)
...
if __name__ == "__main__":
    ...
    save_dir = './save_weights/camelyon16_dsmil_imagenet_100/'
    ...
    for step, name in enumerate(csv_path):
        ...
        train_dsmil((train_dataset, val_dataset, test_dataset),
                     save_path=save_dir,
                     feature_dim = 512,
                     n_classes = 2,
                     fold = step,
