...
def summary(model, loader, n_classes):
    ...
    for batch_idx, (data, label, coords, nearest) in enumerate(loader):
        ...
        with torch.inference_mode():
            logits, Y_prob, Y_hat, y_probs, _ = model(data)
...
def train_mil(datasets,
    ...
    model = MIL_MeanPooling(embed_dim=feature_dim, 
                    n_classes=n_classes)
    _ = model.to(device)
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer, loss_fn):
    ...
    for batch_idx, (data, label, coords, nearest) in enumerate(loader):
        ...
        if (batch_idx + 1) % 20 == 0:
            print('batch {}, loss: {:.4f}, loss_bag: {:.4f}, bag_size: {}'.format(batch_idx, loss_value, loss_bag_value, label.item(), data.size(0)))
...
if __name__ == "__main__":
    csv_path = '/data1/ceiling/workspace/AttriMIL_v2/dataset_csv/unitopatho_train.csv'
    data_dir = '/data2/clh/unitopatho/resnet18_imagenet/'
    split_path = '/data1/ceiling/workspace/AttriMIL_v2/splits/unitopatho/'
    save_dir = './save_weights/unitopatho_mean_imagenet/'
    ...
    dataset = Generic_MIL_Dataset(csv_path = csv_path,
                            ...
                            seed = 1, 
                            print_info = True,
                            label_dict = {'NORM':0, 'HP':1, 'TA.HG':2,'TA.LG':3, 'TVA.HG':4, 'TVA.LG':5},
                            patient_strat=False,
                            ignore=[])
    ...
    for step, name in enumerate(csv_path):
        ...
        train_mil((train_dataset, val_dataset, test_dataset),
                     save_path=save_dir,
                     feature_dim = 512,
                     n_classes = 6,
                     fold = step,
